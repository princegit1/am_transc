...
<%
	try
	{
		...
		stmt=cn.createStatement();
			...
			if(emp_role.equals("AMT"))
			{
				rs=stmt.executeQuery("SELECT * FROM message_details a, category_master b, service_provider c, employee d where a.update_emp_id = d.emp_id and a.category_id=b.category_id and b.service_provider = c.sp_name and a.run_date=curdate() and a.category_id in (9,10,32,40,41) and b.status='ACTIVE'");
			}
			else
			{
...
<%
	try
	{
		...
		if(emp_role.equals("AMT"))
			{
				rs=stmt.executeQuery("select * from category_master where status='ACTIVE' and category_id in (9,10,32,40,41) and category_id not in(SELECT category_id FROM message_details where run_date=curdate())");
			}
			else
			{
