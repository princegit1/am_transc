...
<center>Category Type: <%=categoryType%> &nbsp;&nbsp; | &nbsp;&nbsp; Service Provider: <%=serviceProvider%></center><br>
	...
	<FORM action="insertMessage-onmobile.jsp" method=post>
...
<%
			...
			{	
				...
				while(rs.next())
				{
					...
					{
%>
						<INPUT readOnly value=<%=selectedDate%> name=txtDate<%=i%>> 
<%
					}
					else
					{
...
<%
				...
				{	
					if(select_day == 1)
					{
						upQuery="select msg_id,message"+(k)+",DATE_FORMAT(run_date,'%Y-%m-%d') run_date from message_details_unicode where run_date='"+selectedDate+"' and category_id="+categoryId +" limit "+totalMsg;
					}
					else
					{
						upQuery="select msg_id,message"+(k)+",DATE_FORMAT(run_date,'%Y-%m-%d') run_date from message_details_unicode where run_date='"+selectedDate+"' and category_id="+categoryId;
					}
					out.println("upQuery-->"+upQuery);
					out.println("select_day-->"+select_day);
					stmt=cn.createStatement();
					rs=stmt.executeQuery(upQuery);
					if(rs.next())
					{
