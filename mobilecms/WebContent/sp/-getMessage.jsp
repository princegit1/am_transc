...
<%
	...
	try
	{
		...
		String upQuery1 = "select * from category_master where service_provider='"+serviceProvider+"' and category_type='"+category+"'";
		cn = adminConn.getConnection();
...
<%	
	...
	for(int k=1;k<=totalMsg;k++)
	{
		...
		try
		{	//int q = k+1;
			String upQuery="select header,footer,total_message,COALESCE(message1,'')message1	,COALESCE(message2,'No Message')message2,COALESCE(message3,'No Message')message3,COALESCE(message4,'No Message')message4,COALESCE(message5,'No Message')message5 from message_details a, category_master b where a.category_id=b.category_id and b.service_provider='"+serviceProvider+"' and run_date=CURDATE() and a.category_id="+categoryId;
			//out.println(upQuery);
			//AdminConn adminConn = AdminConn.getInstance();
