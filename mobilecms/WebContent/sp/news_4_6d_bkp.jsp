<%@ page contentType="text/xml;charset=ISO-8859-1"%><%@ page import="java.sql.*,conn.*,java.io.*"%><%@ page import="javax.mail.*" %><%@ page import="javax.mail.internet.*" %><%@page import="java.util.Properties"%><%@page import="java.util.Date"%><%	
	...
	if(category.equalsIgnoreCase("Hollywood-Reports") || category.equalsIgnoreCase("Indian-Cricket")|| category.equalsIgnoreCase("National")|| category.equalsIgnoreCase("Filmi")
	||category.equalsIgnoreCase("ARI") || category.equalsIgnoreCase("TAU")|| category.equalsIgnoreCase("GEM")|| category.equalsIgnoreCase("CAN")
	||category.equalsIgnoreCase("LEO") || category.equalsIgnoreCase("VIR")|| category.equalsIgnoreCase("LIB")|| category.equalsIgnoreCase("SCO")
	||category.equalsIgnoreCase("SAG") || category.equalsIgnoreCase("CAP")|| category.equalsIgnoreCase("AQU")|| category.equalsIgnoreCase("PIS")){
	
	if(category.equalsIgnoreCase("Hollywood-Reports")||category.equalsIgnoreCase("Indian-Cricket")){
	...
	}else if(category.equalsIgnoreCase("Hollywood-Reports") || category.equalsIgnoreCase("Indian-Cricket")|| category.equalsIgnoreCase("National")|| category.equalsIgnoreCase("Filmi")){	
	rs=stmt.executeQuery("select a.message1,date_format(a.RECORD_DATE,'%d%m%Y%T')record_date from message_details a, category_master b where a.category_id=b.category_id and b.service_provider='"+serviceProvider+"' and  status='ACTIVE' and category_type='"+category+"' order by a.MSG_ID  desc limit 1");
	}else{
	 for (int i = 0; i < 12; i++) {
       if(category.equalsIgnoreCase(astroSign[i])){
	...
	if(newsOne.contains(""+category+">")){
	newsOne=newsOne.replaceAll(""+category+">","");
	}
	}
	out.println("<?xml version=\"1.0\" encoding=\"UTF-8\"?>");
    out.println("<data>");
    out.println("<keyword>"+category+"</keyword>"); 
