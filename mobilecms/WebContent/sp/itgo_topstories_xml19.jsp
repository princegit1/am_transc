	...
	%>
			<item>
				...
				<%
					if(paraTag == -1)
			{
				out.println("<p>" + rssBodyText+ "</p>");
			}
			else
			{
				out.println( rssBodyText);
			}
						if(rs.getString("kicker_Image")!=null)
						{
						//	out.println("<img><screen>");
							//out.println("http://media2.intoday.in/itgo/images/stories/"+rs.getString("kicker_Image").replaceAll("&rsquo;", "\'").replaceAll("&lsquo;","\'").replaceAll("&nbsp;"," ").replaceAll("&mdash;"," ").replaceAll("&amp;#39;","").replaceAll("&ldquo;"," ").replaceAll("&rdquo;"," ").replace("&","&amp;").replaceAll("&amp;quot;","").replaceAll("&amp;amp;",""));
