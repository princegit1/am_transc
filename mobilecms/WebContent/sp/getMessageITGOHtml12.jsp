<%@ page import="java.sql.*"%><%@ page import="java.util.*"%><%@ page import="java.util.Date" %><%@ page import ="java.text.SimpleDateFormat"%><%@ page import="conn.*" %><%
 ...
 if(alertDate==null)
	...
	if(username.equals("itgo") & password.equals("itgo") & serviceProvider.equals("ITGO"))
	{
		...
		try
		{	
				upQuery="select date_format(run_date,'%d%m%Y')run_date1, run_date from message_details a, category_master b where a.category_id=b.category_id and b.service_provider='"+serviceProvider+"' and  status='ACTIVE' and category_type='"+category+"' and DATE_FORMAT(run_date, '%d%m%Y')='"+alertDate+"'";
				out.println(upQuery);
