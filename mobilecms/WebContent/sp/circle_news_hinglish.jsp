<%@ page import="java.sql.*,conn.*,com.itgd.server.DecodeXML"%><%String category = "";category = request.getParameter("msg");
		...
		if (msisdn.equals("9999999999")) {
			...
			String[] cCodeArr = {"UPW","UPE","ORI","BHR"};
String[] cNameArr = {"UPW","UPE","ORISSA","BIHAR"};
			Connection cn = null;
			ResultSet rs = null;
			Statement stmt = null;
			...
			if (category != null || !category.equals("")) {
				for (int i = 0; i < 4; i++) {
					...
					if (category.equals(cCodeArr[i])) {
						...
						if(category.equalsIgnoreCase("UPW")){
						out.println(newsOne+"<BR>"+newsTwo+"<BR>"+"Dial*234*82# For  Jokes Rs.15/Month.");
						}else if(category.equalsIgnoreCase("UPE")){
						out.println(newsOne+"<BR>"+newsTwo+"<BR>"+newsThree); 
						}else{
						out.println(newsOne+"<BR>"+newsTwo+"<BR>"+newsThree); 
