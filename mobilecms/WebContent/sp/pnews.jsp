<%@ page import="java.sql.*,conn.*"%><%String category = "PUNJAB";
		String username = request.getParameter("username");
		String password = request.getParameter("password");

		String completeNews = "", newsOne = "", newsTwo = "", newsThree = "";
		String[] cCodeArr = { "AP", "ASM", "BHR", "CHN", "DEL", "GUJ", "HRY",
				"HP", "JK", "KER", "KK", "KOL", "MP", "MAH", "MUM", "NE",
				"ORI", "PUN", "RAJ", "TN", "UPE", "UPW", "WB" };
		String[] cNameArr = { "AP", "ASSAM", "BIHAR", "CHENNAI", "DELHI",
				"Gujarat", "HARYANA", "HP", "JammuKashmir", "KERALA", "KK",
				"KOLKATA", "MP", "Maharastra", "Mumbai", "NORTHEAST", "ORISSA",
				"PUNJAB", "Rajasthan", "TN", "UPEAST", "UPW", "WESTBENGAL" };
		Connection cn = null;
		ResultSet rs = null;
		Statement stmt = null;
		String str = "";
		...
		if (category != null || !category.equals("")) {
			for (int i = 0; i < 23; i++) {
				...
				if (category.equals(cNameArr[i])) {
					completeNews = ab.substring(ab.indexOf(tempStr)
							+ tempStr.length());
					newsOne = completeNews.substring(0, completeNews
							.indexOf("2#"));
					if(newsOne.contains("1#")){
						
						newsOne=newsOne.replace("1#", "1.");
					}
					newsTwo = completeNews.substring(newsOne.length(),
							completeNews.indexOf("3#"));
					if(newsTwo.contains("2#")){
						
						newsTwo=newsTwo.replace("2#", "2.");
					}
					newsThree = completeNews.substring(newsOne.length()
							+ newsTwo.length(), completeNews.length());
		...
		out.println("<part>"+newsOne+"</part>");
		out.println("<part>"+newsTwo+"</part>");
		out.println("<part>"+newsThree+"</part>");
		out.println("</multiPartSms>");
