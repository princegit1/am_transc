...
<%
				...
				{	//int q = k+1;
					...
					if(rs.next())
					{
						...
						{
%>
							<INPUT maxLength=320 size=70 name="txtNews-<%=k%>-<%=i%>" value="<%=rs.getString("message"+k+"")%>" id="eBann-<%=k%>-<%=i%>" onKeyUp="Contar('eBann-<%=k%>-<%=i%>','sBann-<%=k%>-<%=i%>','{CHAR} characters left.',<%=messageSize%>);"> <span id="sBann-<%=k%>-<%=i%>" class="minitext"> <%=messageSize%> characters left.</span>
<%
						}
						else
						{
%>
							<INPUT maxLength=320 size=70 name="txtNews-<%=k%>-<%=i%>" value="" id="eBann-<%=k%>-<%=i%>" onKeyUp="Contar('eBann-<%=k%>-<%=i%>','sBann-<%=k%>-<%=i%>','{CHAR} characters left.',<%=messageSize%>);"> <span id="sBann-<%=k%>-<%=i%>" class="minitext"> <%=messageSize%> characters left.</span>
<%	
						}
					}
					else
