...
{ 
	...
	for(int i=1;i<=select_day;i++)
	{
		...
		for(int k=1;k<=totalMsg;k++)
		{
			...
			//out.println("<br>News:"+i+""+k+" "+new String(new String(txtNews).getBytes("UTF-8"))+"");
			//out.println("<br>News:"+i+""+k+" "+new String(new String(txtNews).getBytes("ISO-8859-1"), "UTF-8")+"");
...
%>
<%!
public void makeXmlFileForVodaFone() {
String fileName="vodafone-loveguru-sms";
String xmlPath="/home/jboss-4.2.2.GA/server/default/deploy/content.war/xml/";
int totalMsg = 0;
int categoryId = 0;	
...
try
{
upQuery="select distinct date_format(run_date,'%d%m%Y')run_date1, run_date ,date_format(run_date,'%m%Y') as run_date2 from message_details_unicode a, category_master b where a.category_id=b.category_id and b.service_provider='"+serviceProvider+"' and  status='ACTIVE' and category_type='"+category+"'  order by run_date";
...
sb.append("<vap version=\"1.0\">");
}
//Tobe changed-monthly content
if(rs3.getString("run_date2").equals("022013")){

sb.append("<contentItem lang=\"en-in\" useOn='"+date+"' expires='"+date+"'>");
...
while(rs.next())
{
sb.append(rs.getString("message1"));
}
...
sb.append("</vap>");
makeXmlFile(sb.toString(),fileName,xmlPath);
}
%>
<%!
