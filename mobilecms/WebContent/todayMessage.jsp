...
<%
				...
				{	
					...
					if(select_day == 1)
					{
						upQuery11="select msg_id,message"+(k)+",DATE_FORMAT(run_date,'%Y-%m-%d') run_date from message_details where run_date='"+date1+"' and category_id="+categoryId;
					}
					else
					{
						upQuery11="select msg_id,message"+(k)+",DATE_FORMAT(run_date,'%Y-%m-%d') run_date from message_details where run_date='"+date+"' and category_id="+categoryId;
					}
					stmt=cn.createStatement();
					rs=stmt.executeQuery(upQuery11);
					if(rs.next())
					{
...
<%
	try
	{
		...
		stmt=cn.createStatement();
		rs=stmt.executeQuery("SELECT * FROM message_details a, category_master b, service_provider c where a.category_id=b.category_id and b.service_provider = c.sp_name and a.run_date=curdate() and a.category_id != 16");
	while(rs.next())
	{
%>
		<tr>
			<td align="right" width="15%"><%=rs.getString("category_type")%></td>
