...
<%
				...
				{	
					...
					if(rs.next())
					{
						...
						{
%>
							<INPUT maxLength=<%=messageSize%> size=70 name="txtNews-<%=k%>-<%=i%>" value="<%=rs.getString("message"+k+"")%>" id="eBann-<%=k%>-<%=i%>" onKeyUp="Contar('eBann-<%=k%>-<%=i%>','sBann-<%=k%>-<%=i%>',' {CHAR} characters left.',<%=messageSize%>);"><span id="sBann-<%=k%>-<%=i%>" class="minitext"><%=messageSize%> characters left.</span><br/>
<%
						}
						else
						{
